...
module Dsfr
  class InputStatusMessageComponent < ApplicationComponent
    ...
    def referentiel_support_statut?
      # pf: also support referentiel_de_polynesie champs
      (type_de_champ.referentiel? || type_de_champ.referentiel_de_polynesie?) && !@champ.idle?
    end
    ...
    def statut_message
      ...
      when TypeDeChamp.type_champs[:siret]
        ...
        elsif @champ.multiple_found? # pf: ambiguous partial Tahiti number
          { state: :info, text: t('.siret.multiple_found') }
        elsif @champ.pending?
      ...
      when TypeDeChamp.type_champs[:referentiel], TypeDeChamp.type_champs[:referentiel_de_polynesie] # pf: include referentiel_de_polynesie
        if @champ.pending?
