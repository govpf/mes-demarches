...
module ChampExternalDataConcern
  ...
  included do
    ...
    aasm column: :external_state, enum: true do
      ...
      event :external_data_multiple_found do
        transitions from: [:fetching], to: :multiple_found
      end
      ...
      event :reset_external_data, after: :after_reset_external_data do
        transitions from: [:idle, :waiting_for_job, :fetching, :fetched, :external_error, :multiple_found], to: :idle
      end
    end
    ...
    def handle_result(result)
      if result.is_a?(Dry::Monads::Result)
        case result
        in Success({ multiple_found: candidates, ** })
          # pf: ambiguous partial Tahiti number matched several etablissements; awaiting user selection
          update_external_data_multiple_found!(candidates)
        in Success(data)
