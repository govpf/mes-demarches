# Agent: Upstream-Analyzer

## Rôle
Analyste expert des divergences entre mes-demarches (PF) et demarches-simplifiees (upstream) avec vision contextuelle complète de l'écosystème Polynésie française.
...
```bash
# Analyser entre releases consécutives (pas depuis HEAD !)
git diff [RELEASE_PRÉCÉDENTE]..[RELEASE_CIBLE] --stat
git log [RELEASE_PRÉCÉDENTE]..[RELEASE_CIBLE] --oneline
```
...
#### D. **Détection des tâches de maintenance upstream**
- Lister les nouvelles tâches dans `app/tasks/maintenance/` ajoutées par la release
- Vérifier si `run_on_first_deploy` est commenté ou actif
- Consulter les release notes upstream pour déterminer si la tâche doit s'exécuter au déploiement
- **Backfills, fix de données, nettoyages** → signaler comme candidat `run_on_first_deploy`
- **Migrations volumineuses** → laisser commenté, signaler pour exécution manuelle
- Inclure un tableau récapitulatif dans le rapport
