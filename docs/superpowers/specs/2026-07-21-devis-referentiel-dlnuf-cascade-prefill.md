...
| **A — Catalogue + cascade** *(prod imminente — Semences)* | Config côté formulaire du champ pilote (colonne du dossier, filtrée sur `type ∈ {text, enum, enums}`) ; cascade ; **validation non-destructive anti-stale** ; état vide simplifié (2 messages, jamais masquer) ; tests système (cascade) + sécurité (request : 403, fail-closed, `q` libre) + unitaires | **~2,5-3 j** | ✅ Livré — branche `feature/referentiel-cascade` (plan `2026-09-14-referentiel-cascade.md`) |
