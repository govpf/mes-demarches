...
describe 'users/dossiers/show/_status_overview', type: :view do
  ...
  context 'when en construction' do
    ...
    it 'works' do
      ...
      expect(subject).to have_text('Voici les délais d’instruction constatés sur les demandes traitées')
      expect(subject).to have_text("Les dossiers les plus rapides sont traités en 1 jour.")
      expect(subject).to have_text("Les dossiers dans la moyenne sont traités en 2 jours.")
      expect(subject).to have_text("Les dossiers les plus longs sont traités en 3 jours.")
    end
  end
