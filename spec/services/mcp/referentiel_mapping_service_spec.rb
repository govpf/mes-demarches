...
RSpec.describe Mcp::ReferentielMappingService do
  ...
  describe '#configurer_source!' do
    ...
    it 'utilise autocomplete par défaut si mode absent' do
      tdc_sans_ref = draft.types_de_champ.find { _1.type_champ == 'referentiel_de_polynesie' }
      tdc_sans_ref.referentiel&.destroy
      tdc_sans_ref.reload
      described_class.new(tdc_sans_ref).configurer_source!(table_id: '24')
      expect(tdc_sans_ref.reload.referentiel.mode).to eq('autocomplete')
    end
  end
