...
RSpec.describe Cron::PurgeOldEmailEventJob, type: :job do
  describe 'perform' do
    ...
    let(:older_than_retention_duration) { create(:email_event, created_at: Time.zone.now - (EmailEvent::RETENTION_DURATION + 1.day)) }
    let(:more_recent_than_retention_duration) { create(:email_event, created_at: Time.zone.now - (EmailEvent::RETENTION_DURATION - 1.day)) }
    before do
