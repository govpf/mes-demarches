    ...
    describe "#process" do
      ...
      context 'when a dossier with missing pj is in construction' do
        ...
        it 'sends an email to the instructeur' do
          expect(BlankMailer).to receive(:send_template).with(hash_including(to: dossier.followers_instructeurs.first.email, subject: "[#{APPLICATION_NAME}] Action requise : pièces jointes manquantes", title: "Pièces jointes manquantes")).and_return(double(deliver_later: true))
          subject
