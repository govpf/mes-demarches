    ...
    describe "#process" do
      ...
      context 'when a dossier with missing pj is in brouillon' do
        ...
        it 'sends an email to the user' do
          expect(BlankMailer).to receive(:send_template).with(hash_including(to: dossier.user.email, subject: "[mes-demarches.gov.pf] Action requise : pièces jointes manquantes", title: "Pièces jointes manquantes")).and_return(double(deliver_later: true))
          subject
