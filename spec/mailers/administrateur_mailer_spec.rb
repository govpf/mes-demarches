...
RSpec.describe AdministrateurMailer, type: :mailer do
  ...
  describe '.notify_service_without_siret' do
    ...
    it do
      expect(subject.to).to eq([admin_email])
      expect(subject.subject).to eq("Numéro TAHITI manquant sur un de vos services")
      expect(subject.body).to include("un de vos services n'a pas son numéro TAHITI renseigné")
    end
